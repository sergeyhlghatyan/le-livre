...
# Parse args first to get connection string for server name
...
parser.add_argument(
    ...
    type=int,
    default=None,
    help="Statement timeout in milliseconds (default: no timeout)",
)
args, _ = parser.parse_known_args()
CONNECTION_STRING: Optional[str] = args.conn

# Determine server name from database in connection string
server_name = "PostgreSQL Explorer"
if CONNECTION_STRING and '/' in CONNECTION_STRING:
    try:
        db_name = CONNECTION_STRING.split('/')[-1].split('?')[0]
        server_name = f"PostgreSQL Explorer ({db_name})"
    except:
        pass

# Initialize FastMCP with unique server name
mcp = FastMCP(
    server_name,
    log_level="INFO"
)

# Optional safety and performance controls
# HARDCODED TO READ-ONLY MODE FOR SAFETY
READONLY: bool = True  # Hardcoded to prevent any write operations
STATEMENT_TIMEOUT_MS: Optional[int] = args.timeout
# Fallback to environment variable if not provided as argument
...
# DEBUG: Write connection details
with open(f"/tmp/mcp_debug_{os.getpid()}.txt", "w") as f:
    f.write(f"PID: {os.getpid()}\n")
    f.write(f"Server Name: {server_name}\n")
    f.write(f"CONNECTION_STRING: {CONNECTION_STRING}\n")
    f.write(f"ENV: {os.getenv('POSTGRES_CONNECTION_STRING')}\n")
